...
class EnhancedContentProcessor:
    ...
    def __init__(self, app_url: str = "http://app:8080", chunk_size: int = 1000, overlap: int = 200):
        """
    ...
    def print_stats(self):
        ...
        print(f"Average Time per Chunk: {self.stats['processing_time']/max(1, self.stats['total_chunks_processed']):.3f} seconds")
    ...
    def process_text_file(self, file_path: str, source_name: str) -> int:
        ...
        try:
            ...
            logger.info(f"📄 Completed processing {os.path.basename(file_path)}: {processed_count}/{len(valid_chunks)} chunks in {processing_time:.2f}s")
    ...
    def process_html_file(self, file_path: str, source_name: str) -> int:
        ...
        try:
            ...
            processing_time = time.time() - start_time
            logger.info(f"🌐 Completed processing {os.path.basename(file_path)}: {processed_count}/{len(chunks)} chunks in {processing_time:.2f}s")
            return processed_count
...
def main():
    ...
    parser.add_argument('--urls-file', type=str, help='Path to file containing URLs to process (one per line)')
